...
import { Basic, mockMoveResults } from './RedactionMoveResultGroup.stories';
import { RedactionPanelContext } from '../RedactionPanel/RedactionPanelContext';

const RedactionMoveResultGroupWithRedux = withProviders(RedactionMoveResultGroup);
...
describe('RedactionMoveResultGroup', () => {
  ...
  describe('component', () => {
    it('renders the correct number of results and page number', () => {
      const selectedMoveResultIndexes = {
        0: false,
        1: false,
        2: false,
        3: false,
      ...
      const setSelectedMoveResultIndexes = jest.fn();
      ...
      customRenderWithContext(<RedactionMoveResultGroupWithRedux {...props} />);
      const redactionMoveResults = screen.getAllByRole('listitem');
    ...
    it('if check a result it calls the right handler', () => {
      ...
      expect(resultCheckBoxes.length).toBe(mockMoveResults.length + 1);
      // Select first result
      userEvent.click(resultCheckBoxes[1]);

      // Notice first page is now selected i.e. true
      expect(setSelectedMoveResultIndexes).toBeCalledWith({
        0: true,
        1: false,
        2: false,
        3: false,
