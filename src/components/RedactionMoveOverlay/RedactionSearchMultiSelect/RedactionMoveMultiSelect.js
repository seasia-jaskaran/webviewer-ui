...
const RedactionMoveMultiSelect = (props) => {
  const { t } = useTranslation();
  const { activeTheme, redactionMoveOptions } = props;

  const redactionGroup = [
    {
      label: t('redactionPanel.move.pattern'),
      options: redactionMoveOptions,
    },
  ];
  ...
  return (
    <CreatableMultiSelect
      ...
      placeholder={t('redactionPanel.redactionMovePlaceholder')}
      formatCreateLabel={(value) => `${t('component.movePanel')} ${value}`}
      {...props}
    />
  );
};
